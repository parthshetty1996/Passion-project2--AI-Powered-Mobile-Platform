...
with tabs[2]:
    ...
    perf = pd.DataFrame({n:{
        ...
        "F1":f"{v['f1']:.3f}","AUC":f"{v['auc']:.3f}" if v['auc'] else "N/A"
    ...
    st.markdown(f"""<div class='insight-box'>
    <b>🏆 Best Model: {best_m}</b> — F1={clf_res[best_m]['f1']:.3f},
    AUC={clf_res[best_m]['auc']:.3f if clf_res[best_m]['auc'] else 'N/A'}
    </div>""", unsafe_allow_html=True)
